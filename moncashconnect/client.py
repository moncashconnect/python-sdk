...
_BASE_URL = "https://api.moncashconnect.com/v1"
_SDK_VERSION = "1.0.0"
...
class MonCashClient:
    ...
    Args:
        secret_key: Your project secret key (starts with ``sk_proj_``).
    ...
    def __init__(self, secret_key: str, timeout: int = 20) -> None:
        if not secret_key.startswith("sk_proj_"):
            raise MonCashError('Secret key must start with "sk_proj_"')
    ...
    def create_payment(
        ...
        *,
        ...
        """
        payload: dict[str, Any] = {"amount": amount, "referenceId": reference_id}
        if return_url:
        ...
        return self._request("POST", "/pay-create", payload)
    ...
    def _request(
        ...
        if status < 200 or status >= 300:
            ...
            raise MonCashError(str(message), status, data_resp)
